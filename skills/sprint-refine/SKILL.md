...
The Verifier reads both parts in Phase 1 (part (a) as constraints to enforce, part (b) to ground absence claims) and **computes** the refreshed part-(b) inventory, returning it in its output; the **Manager writes** the memory files (they live under `~/.sprint-refine/`, outside any repo; the Verifier's own write scope stays none). Promotion INTO part (a) happens at close-out — see Phase 4.
...
2. **Large task** (median > 3 SP): Leader breaks it into subtasks → back to Phase 2 for the new ones → re-vote.
   - **Anti-loop cap: 2 *unproductive* break cycles per task.** The counter measures lack of *progress*, not lack of *change*:
     ...
     - Churn (**consumes one slot**): a re-vote that leaves the task text and the task count unchanged (same task rewritten back and forth).
   - After 2 slots with no progress, mark the task `[NEEDS-SPLIT]` and continue.
...
Task breaking has two independent triggers, in order:

- **Phase 2 (Leader): natural units.** A new screen/route/entrypoint becomes its own task regardless of SP — small-but-natural units are separated before any voting.
- **Phase 3 (SP): the `>3` median.** An *additive* trigger that catches whatever is still too big after the natural-unit split.
...
- Break to small **natural units** (new screen/route/entrypoint → own task) in Phase 2, even below 3 SP; recommended backend unit = an entrypoint-to-database slice (enables DB/API integration tests); don't go all the way to one-task-per-function. The `>3 SP` median break in Phase 3 is an additive trigger, not the only one. A single unit slightly more complex than usual may stand alone as its own task under 3 SP.
...
- Persistent **project memory** per target repo at `~/.sprint-refine/project-memory/<repo>-{be,fe}.md` (under the user's home, outside any git checkout — immune to accidental commit and to the skill being copied between repos). Part (a) recurring patterns/conventions (admission: user-enunciated pattern OR ≥2× recurrence; obsolescence only via a new user FIX, never auto-expire); part (b) user-visible-capability feature inventory stamped to a commit hash, updated incrementally. Verifier reads it in Phase 1; patterns promoted at close-out.
- Sprint carries a `## Design decisions` log (canonical English title; localized titles like `## Decisões de design` are recognized as the same section — option b, co-located, not a separate file): Leader reads it as **binding** at the start of each round and appends new decisions; it feeds the memory promotion at close-out; the human publisher deletes it (harmless if forgotten).
