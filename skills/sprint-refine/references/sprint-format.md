...
A bare title is not enough — **every task carries at least a one-line description** of what it is / what it must accomplish, as a sub-item under the title (the title is the scannable "what"; the description gives the dev the "so they can build it" detail). This applies to *every* task, including the small ones generated by breaking a larger task down (e.g. the individual tasks of a template checklist get a per-task description, not just a title). A task that is only a title is incomplete.
