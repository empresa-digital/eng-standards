...
- **Context outside the hierarchy** — a comment (often a blockquote) placed at the **start of a section** that really refers to one specific task → move it onto that task's first line or a sub-item. But a `>` blockquote nested as a **per-task sub-item**, carrying an authorial note/aside scoped to that one task, is legitimate — **preserve it**. Only relocate/strip a per-task `>` if it references something beyond its own task. Section-level blockquotes for context on a larger feature remain fine, and only rarely.
- **Unqualified code names** — a bare symbol that could live in more than one layer (`CreateDiligence` could be the handler, the repo method, or the route) → add a short kind-word qualifier: "the handler `X`", "`repo.X`", "the entity `Y`", "the `Z` table/column", "the listing route", "the X service/helper/page/component". When one value crosses layers under different spellings (body param → entity field → DB column), say so explicitly once.
- **Feature blockquote that opens with structure** → a `>` intro for a larger feature must open with the **problem it solves in the user's/flow's terms**, then move to the solution and its technical structure. If it opens with the structural distinction (e.g. "new entity X, not an extension of Y"), rewrite it problem-first. Verbose-and-clear beats short-and-obtuse here.
