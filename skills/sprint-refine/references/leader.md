...
- **Context stays inside the item hierarchy.** Anything that references a specific task goes on that task's first line or a sub-item — never a section-level comment/blockquote. A `>` blockquote is allowed as a **per-task sub-item** (authorial note/aside scoped strictly to that one task) — preserve those; and rarely at section level for context on a larger feature (see `references/sprint-format.md`).
